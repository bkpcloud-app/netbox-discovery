...
def execute(apply_mode):
    ...
    try:
        ...
        if apply_mode:
            run_step("IMPORT_FINALIZE", [py, os.path.join(BASE, "modules/importers/importer_v6.py"), "--apply"], stages)
            run_step("AUDIT_FINALIZE", [py, os.path.join(BASE, "modules/auditors/auditor_v6.py")], stages)
