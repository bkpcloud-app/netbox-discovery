...
def _single_interface_fallback(row, asset, vm, live_state, original_candidate, classification_rows):
    """Allow a safe repair when the unique VM interface has no MAC object yet.

    This is deliberately narrower than the normal MAC match path:
    - exactly one strong VMware MAC for the asset;
    - exactly one VM already selected uniquely by name by planner_v3/v4;
    - exactly one live interface on that VM;
    - that interface has no other MAC identity;
    - the MAC is absent globally, unassigned, or already on that interface.

    The base planner still validates all Device/IP/product-ownership protections.
    We only synthesize the missing VM-interface MAC evidence for planning and mark
    the repair so importer_v6 can create/assign the MAC before moving the IP.
    """
    ...
    if len(strong_macs) != 1:
        return None, "Fallback de interface única exige exatamente um MAC VMware forte: {0}".format(len(strong_macs))
    vm_mac = strong_macs[0]

    vm_interfaces = v4._vm_interfaces(live_state, vm.get("id"))
    if len(vm_interfaces) != 1:
        return None, "Fallback de interface única exige exatamente uma interface na VM: {0}".format(len(vm_interfaces))
    target = vm_interfaces[0]

    target_live_macs = v4._vm_interface_macs(target, live_state)
    if target_live_macs:
        return None, "Interface única da VM já possui MAC diferente: {0}".format(",".join(sorted(target_live_macs)))
    ...
    if global_matches:
        ...
        if assigned_id and (assigned_type != "virtualization.vminterface" or assigned_id != target.get("id")):
            return None, "MAC VMware pertence a outro objeto: {0} ID {1}".format(assigned_type or "outro", assigned_id)
...
def build_plan(recon, classification, state):
    ...
    def repair_candidate(row, asset, vm, live_state):
        return _single_interface_fallback(
