...
def generate_fresh_plan():
    ...
    if not rows:
        raise RuntimeError("PLAN V9 não gerou JSON para idempotência")
    return rows[0]


def main(argv=None):
    old_generate_v7 = v7.generate_fresh_plan
    old_generate_v8 = v8.generate_fresh_plan
    old_version = v7.AUDITOR_VERSION
    try:
        v7.generate_fresh_plan = generate_fresh_plan
        v8.generate_fresh_plan = generate_fresh_plan
        v7.AUDITOR_VERSION = AUDITOR_VERSION
        return v8.main(argv)
    finally:
        v7.AUDITOR_VERSION = old_version
        v8.generate_fresh_plan = old_generate_v8
        v7.generate_fresh_plan = old_generate_v7
